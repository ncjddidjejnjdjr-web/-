...
ADMIN_TARGET = os.getenv("ADMIN_TARGET", "7809557665")  # اینجا عدد بذارید، نه @
...
# تابع پاکسازی هوشمند متن (برای حذف کدها و علائم عجیب)
...
async def send_joined_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """گزارش فوری به ادمین وقتی کاربر استارت رو می‌زنه"""
...
async def start_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # پرامپت سختگیرانه (فقط ۱ سوال فارسی، بدون کد)
    system_prompt = (
    ...
    # ===== انیمیشن تایپ کردن (قبل از پیام آماده‌سازی) =====
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== انیمیشن تایپ کردن (قبل از بررسی هوش مصنوعی) =====
    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
    ...
    if is_win:
        ...
        try:
            ...
            await update.message.reply_text(f"🎉 **تبریک! گزینه صحیح رو زدی!**\n\n🏆 **جایزه (لینک کانال):**\n{link.invite_link}")
        except:
        ...
        await update.message.reply_text(f"❌ **باختی!** گزینه درست رو انتخاب نکردی.\n🧠 {clean_user_msg}\n\nبرای تلاش مجدد، /start رو بزن.")
        context.user_data.clear()
        return ConversationHandler.END
...
if __name__ == '__main__':
    ...
    if WEBHOOK_URL:
        print("ربات نهایی با انیمیشن روشن شد!")
